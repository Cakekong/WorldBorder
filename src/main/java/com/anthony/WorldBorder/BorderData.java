...
public class BorderData {
	 ...
	 * 				- z : y coordonate of central point of polygon
	 * 				- pointsx : tab contains X coordonates of polygon
	...
	public Location correctedPosition(Location loc, boolean round,
		...
		// round border
			...
			double dU = Math.sqrt(dX * dX + dZ * dZ); // distance of the
														// untransformed point
														// from the center
			double dT = Math.sqrt(dX * dX / radiusXSquared + dZ * dZ
					/ radiusZSquared); // distance of the transformed point from
										// the center
		...
		yLoc = getSafeY(loc.getWorld(), ixLoc, Location.locToBlock(yLoc),
				izLoc, flying);
		if (yLoc == -1)
			return null;
	...
	// these material IDs are acceptable for places to teleport player;
	// breathable blocks and water
	public static final LinkedHashSet<Integer> safeOpenBlocks = new LinkedHashSet<Integer>(
			Arrays.asList(new Integer[] { 0, 6, 8, 9, 27, 28, 30, 31, 32, 37,
					38, 39, 40, 50, 55, 59, 63, 64, 65, 66, 68, 69, 70, 71, 72,
					75, 76, 77, 78, 83, 90, 93, 94, 96, 104, 105, 106, 115,
					131, 132, 141, 142, 149, 150, 157 }));

	// these material IDs are ones we don't want to drop the player onto, like
	// cactus or lava or fire or activated Ender portal
	public static final LinkedHashSet<Integer> painfulBlocks = new LinkedHashSet<Integer>(
			Arrays.asList(new Integer[] { 10, 11, 51, 81, 119 }));

	// check if a particular spot consists of 2 breathable blocks over something
	// relatively solid
	private boolean isSafeSpot(World world, int X, int Y, int Z, boolean flying) {
		boolean safe = safeOpenBlocks.contains((Integer) world
				.getBlockTypeIdAt(X, Y, Z)) // target block open and safe
				&& safeOpenBlocks.contains((Integer) world.getBlockTypeIdAt(X,
						Y + 1, Z)); // above target block open and safe
		if (!safe || flying)
			return safe;

		Integer below = (Integer) world.getBlockTypeIdAt(X, Y - 1, Z);
		return (safe
				&& (!safeOpenBlocks.contains(below) || below == 8 || below == 9) // below
																					// target
																					// block
																					// not
																					// open/breathable
	...
	private double getSafeY(World world, int X, int Y, int Z, boolean flying) {
		...
		final int limTop = (world.getEnvironment() == World.Environment.NETHER) ? 125
		...
		return -1.0; // no safe Y location?!?!? Must be a rare spot in a Nether
						// world or something
	}
