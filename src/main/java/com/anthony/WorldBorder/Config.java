...
public class Config {
	...
	private static Set<String> bypassPlayers = Collections
	...
	private static Map<String, BorderData> borders = Collections
			.synchronizedMap(new LinkedHashMap<String, BorderData>());
	...
	public static void setBorder(String world, int radiusX, int radiusZ,
			double x, double z, Boolean shapeRound) {
	...
	public static void setBorderCorners(String world, double x1, double z1,
			double x2, double z2, Boolean shapeRound, boolean wrap) {
		...
		setBorder(world, new BorderData(x, z, (int) Math.round(radiusX),
				(int) Math.round(radiusZ), shapeRound, wrap));
	}

	public static void setBorderCorners(String world, double x1, double z1,
			double x2, double z2, Boolean shapeRound) {
	...
	public static void StartBorderTimer() {
		...
		borderTask = plugin
				.getServer()
				.getScheduler()
				.scheduleSyncRepeatingTask(plugin, new BorderCheckTask(),
	...
	public static void RestoreFillTask(String world, int fillDistance,
		...
		fillTask = new WorldFillTask(plugin.getServer(), null, world,
				fillDistance, chunksPerRun, tickFrequency);
		if (fillTask.valid()) {
	...
	public static void load(WorldBorder master, boolean logIt) { // load config
																	// from file
		plugin = master;
		...
		if (message == null || message.isEmpty()) { // store defaults
		...
		if (worlds != null) {
			...
			for (String worldName : worldNames) {
				...
				// we're swapping "<" to "." at load since periods denote
				// configuration nodes without a working way to change that, so
				// world names with periods wreak havoc and are thus modified
				// for storage
				if (cfgVersion > 3)
					worldName = worldName.replace("<", ".");

				// backwards compatibility for config from before
				// elliptical/rectangular borders were supported
				...
				BorderData border = new BorderData(bord.getDouble("x", 0),
						bord.getDouble("z", 0), bord.getInt("radiusX", 0),
		...
		// if we have an unfinished fill task stored from a previous run, load
		// it up
	...
	public static void save(boolean logIt, boolean storeFillTask) { // save
																	// config to
																	// file
		if (cfg == null)
			return;
