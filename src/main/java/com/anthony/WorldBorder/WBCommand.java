...
public class WBCommand implements CommandExecutor {
	...
	private final String clrOpt = ChatColor.DARK_GREEN.toString(); // optional
																	// values
	private final String clrDesc = ChatColor.WHITE.toString(); // command
																// descriptions
	private final String clrHead = ChatColor.YELLOW.toString(); // command
																// listing
																// header
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		String cmdW = clrCmd
				+ ((player == null) ? "wb " + clrReq + "<world>" : "/wb "
						+ clrOpt + "[world]") + clrCmd;
		...
		if ((split.length == 5 || split.length == 6)
			...
			if (world == null)
				sender.sendMessage("The world you specified (\""
						+ split[0]
						+ "\") could not be found on the server, but data for it will be stored anyway.");

			if (cmdSet(sender, split[0], split, 2, (split.length == 5))
					&& player != null)
				sender.sendMessage("Border has been set. "
						+ Config.BorderDescription(split[0]));
		}
		// "set" command from player, using current world, X and Z specified
		else if ((split.length == 4 || split.length == 5)
				&& split[0].equalsIgnoreCase("set") && player != null) {
			if (!Config.HasPermission(player, "set"))
				return true;
		...
		// "set" command from player, using current world, X and Z specified and
		// points coordo
		else if ((split.length == 4 || split.length == 5)
				&& split[0].equalsIgnoreCase("set") && player != null) {
			if (!Config.HasPermission(player, "set"))
		...
		// "set" command from player, using current world, X and Z NOT specified
			...
			try {
				...
				sender.sendMessage(clrErr
						+ "The radius value(s) must be integers.");
				return true;
			}
		...
		// "setcorners" command from player or console, world specified
			...
			try {
				...
				sender.sendMessage(clrErr
						+ "The x1, z1, x2, and z2 values must be numerical.");
				return true;
			}
		...
		// "radius" command from player or console, world specified
				...
				&& split[1].equalsIgnoreCase("radius")) {
			...
			if (border == null) {
				sender.sendMessage(clrErr + "That world (\"" + world
						+ "\") must first have a border set normally.");
				return true;
			}
			...
			if (player != null)
				sender.sendMessage("Radius has been set. "
						+ Config.BorderDescription(world));
		}

		// "radius" command from player, using current world
		else if ((split.length == 2 || split.length == 3)
				&& split[0].equalsIgnoreCase("radius") && player != null) {
			if (!Config.HasPermission(player, "radius"))
				return true;
			...
			BorderData border = Config.Border(world);
			if (border == null) {
				sender.sendMessage(clrErr + "This world (\"" + world
						+ "\") must first have a border set normally.");
				return true;
			}
		...
		// "clear" command from player or console, world specified
			...
			BorderData border = Config.Border(world);
			if (border == null) {
				sender.sendMessage("The world you specified (\"" + world
						+ "\") does not have a border set.");
				return true;
			}
		...
		// "clear" command from player, using current world
			...
			BorderData border = Config.Border(world);
			if (border == null) {
				sender.sendMessage(clrErr + "Your current world (\"" + world
						+ "\") does not have a border set.");
				return true;
			}
		...
		else if (split.length == 2 && split[0].equalsIgnoreCase("clear")
		...
		// "list" command from player or console
			...
			sender.sendMessage("Default border shape for all worlds is \""
					+ Config.ShapeName() + "\".");
		...
		// "shape" command from player or console
			...
			if (player != null)
				sender.sendMessage("Default border shape for all worlds is now set to \""
						+ Config.ShapeName() + "\".");
		}
		...
		// "reload" command from player or console
			...
			if (player != null)
				Config.Log("Reloading config file at the command of player \""
						+ player.getName() + "\".");
		...
		// "debug" command from player or console
			...
			if (player != null)
				Config.Log((Config.Debug() ? "Enabling" : "Disabling")
						+ " debug output at the command of player \""
						+ player.getName() + "\".");

			if (player != null)
				sender.sendMessage("Debug mode "
						+ enabledColored(Config.Debug()) + ".");
		}
		...
		// "whoosh" command from player or console
			...
			if (player != null) {
				Config.Log((Config.whooshEffect() ? "Enabling" : "Disabling")
						+ " \"whoosh\" knockback effect at the command of player \""
						+ player.getName() + "\".");
				sender.sendMessage("\"Whoosh\" knockback effect "
						+ enabledColored(Config.whooshEffect()) + ".");
			}
		}
		...
		// "knockback" command from player or console
			...
			try {
				...
				sender.sendMessage(clrErr
						+ "The knockback must be a decimal value of at least 1.0, or it can be 0.");
				return true;
			}
			...
			if (player != null)
				sender.sendMessage("Knockback set to " + numBlocks
						+ " blocks inside the border.");
		}
		...
		// "delay" command from player or console
			...
			try {
				...
				sender.sendMessage(clrErr
						+ "The timer delay must be an integer of 1 or higher.");
				return true;
			}
			if (delay < 1) {
				sender.sendMessage(clrErr
			...
			if (player != null)
				sender.sendMessage("Timer delay set to " + delay
						+ " tick(s). That is roughly " + (delay * 50) + "ms.");
		}
		...
		// "wshape" command from player or console, world specified
			...
			if (split[2].equalsIgnoreCase("rectangular")
					|| split[2].equalsIgnoreCase("square"))
				shape = false;
			else if (split[2].equalsIgnoreCase("elliptic")
					|| split[2].equalsIgnoreCase("round"))
				shape = true;
			...
			if (player != null)
				sender.sendMessage("Border shape for world \""
						+ world
						+ "\" is now set to \""
						+ (shape == null ? "default" : Config.ShapeName(shape
								.booleanValue())) + "\".");
		}

		// "wshape" command from player, using current world
		else if (split.length == 2 && split[0].equalsIgnoreCase("wshape")
				&& player != null) {
			if (!Config.HasPermission(player, "wshape"))
				return true;
			...
			BorderData border = Config.Border(world);
			if (border == null) {
				sender.sendMessage("This world (\"" + world
						+ "\") does not have a border set.");
				return true;
			}
		...
		// "wrap" command from player or console, world specified
			...
			if (player != null)
				sender.sendMessage("Border for world \"" + world
						+ "\" is now set to " + (wrap ? "" : "not ")
						+ "wrap around.");
		}

		// "wrap" command from player, using current world
		else if (split.length == 2 && split[0].equalsIgnoreCase("wrap")
				&& player != null) {
			if (!Config.HasPermission(player, "wrap"))
				return true;
		...
		// "portal" command from player or console
			...
			if (player != null) {
				Config.Log((Config.portalRedirection() ? "Enabling"
						: "Disabling")
						+ " portal redirection at the command of player \""
						+ player.getName() + "\".");
				sender.sendMessage("Portal redirection "
						+ enabledColored(Config.portalRedirection()) + ".");
			}
		}
		...
		// "fill" command from player or console, world specified
			...
			cmdFill(sender, player, world, confirm, cancel, pause, pad,
					frequency);
		}

		// "fill" command from player (or from console solely if using cancel or
		// confirm), using current world
			...
			if (!cancel && !confirm && !pause && world.isEmpty()) {
				sender.sendMessage("You must specify a world! Example: " + cmdW
						+ " fill " + clrOpt + "[freq] [pad]");
				return true;
			}
		...
		// "trim" command from player or console, world specified
			...
			cmdTrim(sender, player, world, confirm, cancel, pause, pad,
					frequency);
		}

		// "trim" command from player (or from console solely if using cancel or
		// confirm), using current world
		...
		// "dynmap" command from player or console
			...
			sender.sendMessage("DynMap border display "
					+ (Config.DynmapBorderEnabled() ? "enabled" : "disabled")
					+ ".");

			if (player != null)
				Config.Log((Config.DynmapBorderEnabled() ? "Enabled"
						: "Disabled")
		...
		// "bypass" command from player or console, player specified, on/off
		// optionally specified
			...
			if (target != null && target.isOnline())
				target.sendMessage("Border bypass is now "
						+ enabledColored(bypassing) + ".");

			Config.Log("Border bypass for player \""
					+ sPlayer
					+ "\" is "
					+ (bypassing ? "enabled" : "disabled")
					+ (player != null ? " at the command of player \""
							+ player.getName() + "\"" : "") + ".");
			if (player != null && player != target)
				sender.sendMessage("Border bypass for player \"" + sPlayer
						+ "\" is " + enabledColored(bypassing) + ".");
		}

		// "bypass" command from player, using them for player
		else if (split.length == 1 && split[0].equalsIgnoreCase("bypass")
				&& player != null) {
			if (!Config.HasPermission(player, "bypass"))
				return true;
			...
			Config.Log("Border bypass is "
					+ (bypassing ? "enabled" : "disabled") + " for player \""
					+ sPlayer + "\".");
		...
		// we couldn't decipher any known commands, so show help
			...
			sender.sendMessage(clrHead + plugin.getDescription().getFullName()
					+ " - commands (" + clrReq + "<required> " + clrOpt
					+ "[optional]" + clrHead + ")"
					+ (page > 0 ? " " + page + "/4" : "") + ":");

			if (page == 0 || page == 1) {
				if (player != null)
					sender.sendMessage(cmd + " set " + clrReq + "<radiusX> "
							+ clrOpt + "[radiusZ]" + clrDesc
							+ " - set border, centered on you.");
				sender.sendMessage(cmdW + " set " + clrReq + "<radiusX> "
						+ clrOpt + "[radiusZ] <x> <z>" + clrDesc
						+ " - set border.");
				sender.sendMessage(cmdW + " setcorners " + clrReq
						+ "<x1> <z1> <x2> <z2>" + clrDesc
						+ " - set by corners.");
				sender.sendMessage(cmdW + " radius " + clrReq + "<radiusX> "
						+ clrOpt + "[radiusZ]" + clrDesc + " - change radius.");
				sender.sendMessage(cmdW + " clear" + clrDesc
						+ " - remove border for this world.");
				sender.sendMessage(cmd + " clear all" + clrDesc
						+ " - remove border for all worlds.");
				sender.sendMessage(cmd + " shape " + clrReq
						+ "<elliptic|rectangular>" + clrDesc
						+ " - set the default shape.");
				sender.sendMessage(cmd + " shape " + clrReq + "<round|square>"
						+ clrDesc + " - same as above.");
				if (page == 1)
					sender.sendMessage(cmd + " 2" + clrDesc
							+ " - view second page of commands.");
			}
			if (page == 0 || page == 2) {
				sender.sendMessage(cmd + " list" + clrDesc
						+ " - show border information for all worlds.");
				sender.sendMessage(cmdW + " fill " + clrOpt + "[freq] [pad]"
						+ clrDesc + " - generate world out to border.");
				sender.sendMessage(cmdW + " trim " + clrOpt + "[freq] [pad]"
						+ clrDesc + " - trim world outside of border.");
				sender.sendMessage(cmd
						+ " bypass "
						+ ((player == null) ? clrReq + "<player>" : clrOpt
								+ "[player]") + clrOpt + " [on/off]" + clrDesc
						+ " - let player go beyond border.");
				sender.sendMessage(cmd
						+ " wshape "
						+ ((player == null) ? clrReq + "<world>" : clrOpt
								+ "[world]") + clrReq
						+ " <elliptic|rectangular|default>" + clrDesc
						+ " - shape override for this world.");
				// above command takes 2 lines, so only 7 commands total listed
				// for this page
				sender.sendMessage(cmd
						+ " wshape "
						+ ((player == null) ? clrReq + "<world>" : clrOpt
								+ "[world]") + clrReq
						+ " <round|square|default>" + clrDesc
						+ " - same as above.");
				sender.sendMessage(cmd
						+ " wrap "
						+ ((player == null) ? clrReq + "<world>" : clrOpt
								+ "[world]") + clrReq + " <on/off>" + clrDesc
						+ " - can make border crossings wrap.");
				if (page == 2)
					sender.sendMessage(cmd + " 3" + clrDesc
							+ " - view third page of commands.");
			}
			if (page == 0 || page == 3) {
				sender.sendMessage(cmd + " whoosh " + clrReq + "<on|off>"
						+ clrDesc + " - turn knockback effect on or off.");
				sender.sendMessage(cmd + " portal " + clrReq + "<on|off>"
						+ clrDesc + " - turn portal redirection on or off.");
				sender.sendMessage(cmd + " getmsg" + clrDesc
						+ " - display border message.");
				sender.sendMessage(cmd + " setmsg " + clrReq + "<text>"
						+ clrDesc + " - set border message.");
				sender.sendMessage(cmd + " knockback " + clrReq + "<distance>"
						+ clrDesc + " - how far to move the player back.");
				sender.sendMessage(cmd + " delay " + clrReq + "<amount>"
						+ clrDesc + " - time between border checks.");
				sender.sendMessage(cmd + " dynmap " + clrReq + "<on|off>"
						+ clrDesc + " - turn DynMap border display on or off.");
				sender.sendMessage(cmd + " dynmapmsg " + clrReq + "<text>"
						+ clrDesc + " - DynMap border labels will show this.");
				if (page == 3)
					sender.sendMessage(cmd + " 4" + clrDesc
							+ " - view fourth page of commands.");
			}
			if (page == 0 || page == 4) {
				sender.sendMessage(cmd + " reload" + clrDesc
						+ " - re-load data from config.yml.");
				sender.sendMessage(cmd + " debug " + clrReq + "<on|off>"
						+ clrDesc + " - turn console debug output on or off.");
				if (page == 4)
					sender.sendMessage(cmd + clrDesc
	...
	private void fillDefaults() {
		fillWorld = "";
		fillFrequency = 20;
		// with "view-distance=10" in server.properties and
		// "Render Distance: Far" in client, hitting border during testing
		// was loading 11 chunks beyond the border in a couple of directions (10
		// chunks in the other two directions); thus:
		fillPadding = CoordXZ.chunkToBlock(13);
	}

	private boolean cmdFill(CommandSender sender, Player player, String world,
			boolean confirm, boolean cancel, boolean pause, String pad,
			String frequency) {
		if (cancel) {
			sender.sendMessage(clrHead
		...
		if (pause) {
			if (Config.fillTask == null || !Config.fillTask.valid()) {
				sender.sendMessage(clrHead
						+ "The world map generation task is not currently running.");
				return true;
			}
		...
		if (confirm) { // command confirmed, go ahead with it
			...
			Config.fillTask = new WorldFillTask(plugin.getServer(), player,
					fillWorld, fillPadding, repeats, ticks);
			if (Config.fillTask.valid()) {
				int task = plugin
						.getServer()
						.getScheduler()
						.scheduleSyncRepeatingTask(plugin, Config.fillTask,
								ticks, ticks);
				Config.fillTask.setTaskID(task);
				sender.sendMessage("WorldBorder map generation for world \""
						+ fillWorld + "\" task started.");
			} else
				sender.sendMessage(clrErr
			...
			if (fillWorld.isEmpty()) {
				sender.sendMessage(clrErr
						+ "You must first specify a valid world.");
				return false;
			}

			String cmd = clrCmd + ((player == null) ? "wb" : "/wb");
			sender.sendMessage(clrHead
					+ "World generation task is ready for world \"" + fillWorld
					+ "\", padding the map out to " + fillPadding
					+ " blocks beyond the border (default "
					+ CoordXZ.chunkToBlock(13)
					+ "), and the task will try to generate up to "
					+ fillFrequency + " chunks per second (default 20).");
			sender.sendMessage(clrHead
					+ "This process can take a very long time depending on the world's border size. Also, depending on the chunk processing rate, players will likely experience severe lag for the duration.");
			sender.sendMessage(clrDesc + "You should now use " + cmd
					+ " fill confirm" + clrDesc + " to start the process.");
			sender.sendMessage(clrDesc + "You can cancel at any time with "
					+ cmd + " fill cancel" + clrDesc
					+ ", or pause/unpause with " + cmd + " fill pause"
					+ clrDesc + ".");
		}
		return true;
	}
	...
	private boolean cmdTrim(CommandSender sender, Player player, String world,
			boolean confirm, boolean cancel, boolean pause, String pad,
			String frequency) {
		if (cancel) {
			sender.sendMessage(clrHead
					+ "Cancelling the world map trimming task.");
			trimDefaults();
			Config.StopTrimTask();
			return true;
		}
		...
		if (pause) {
			if (Config.trimTask == null || !Config.trimTask.valid()) {
				sender.sendMessage(clrHead
						+ "The world map trimming task is not currently running.");
				return true;
			}
			Config.trimTask.pause();
			sender.sendMessage(clrHead + "The world map trimming task is now "
					+ (Config.trimTask.isPaused() ? "" : "un") + "paused.");
			return true;
		}

		if (Config.trimTask != null && Config.trimTask.valid()) {
			sender.sendMessage(clrHead
					+ "The world map trimming task is already running.");
			return true;
		}
		...
		if (confirm) { // command confirmed, go ahead with it
			...
			if (player != null)
				Config.Log("Trimming world beyond border at the command of player \""
						+ player.getName() + "\".");
			...
			Config.trimTask = new WorldTrimTask(plugin.getServer(), player,
					trimWorld, trimPadding, repeats);
			if (Config.trimTask.valid()) {
				int task = plugin
						.getServer()
						.getScheduler()
						.scheduleSyncRepeatingTask(plugin, Config.trimTask,
								ticks, ticks);
				Config.trimTask.setTaskID(task);
				sender.sendMessage("WorldBorder map trimming task for world \""
						+ trimWorld + "\" started.");
			} else
				sender.sendMessage(clrErr
						+ "The world map trimming task failed to start.");
			...
			String cmd = clrCmd + ((player == null) ? "wb" : "/wb");
			sender.sendMessage(clrHead
					+ "World trimming task is ready for world \"" + trimWorld
					+ "\", trimming the map past " + trimPadding
					+ " blocks beyond the border (default "
					+ CoordXZ.chunkToBlock(13)
					+ "), and the task will try to process up to "
					+ trimFrequency + " chunks per second (default 5000).");
			sender.sendMessage(clrHead
					+ "This process can take a while depending on the world's overall size. Also, depending on the chunk processing rate, players may experience lag for the duration.");
			sender.sendMessage(clrDesc + "You should now use " + cmd
					+ " trim confirm" + clrDesc + " to start the process.");
			sender.sendMessage(clrDesc + "You can cancel at any time with "
					+ cmd + " trim cancel" + clrDesc
					+ ", or pause/unpause with " + cmd + " trim pause"
					+ clrDesc + ".");
		}
		return true;
	}
