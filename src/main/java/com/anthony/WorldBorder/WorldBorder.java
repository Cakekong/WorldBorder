...
public class WorldBorder extends JavaPlugin {
	...
	public void onEnable() {
		...
		// our one real command, though it does also have aliases "wb" and
		// "worldborder"
		getCommand("wborder").setExecutor(new WBCommand(this));

		// keep an eye on teleports, to redirect them to a spot inside the
		// border if necessary
		...
		System.out
				.println("For reference, the main world's spawn location is at X: "
						+ Config.coord.format(spawn.getX())
						+ " Y: "
						+ Config.coord.format(spawn.getY())
						+ " Z: "
						+ Config.coord.format(spawn.getZ()));
	}
