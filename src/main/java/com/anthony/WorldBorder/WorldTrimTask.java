...
public class WorldTrimTask implements Runnable {
	...
	private transient int currentRegion = -1; // region(file) we're at in
												// regionFiles
	...
	private transient int currentChunk = 0; // chunk we've reached in the
											// current region (regionChunks)
	...
	public WorldTrimTask(Server theServer, Player player, String worldName,
			int trimDistance, int chunksPerRun) {
		...
		this.border = (Config.Border(worldName) == null) ? null : Config
		...
		// each region file covers up to 1024 chunks; with all operations we
		// might need to do, let's figure 3X that
	...
	public void run() {
		...
		while (counter <= chunksPerRun) {
			...
			if (regionChunks.isEmpty())
				addCornerChunks();
			else if (currentChunk == 4) { // determine if region is completely
											// _inside_ border based on corner
											// chunks
				if (trimChunks.isEmpty()) { // it is, so skip it and move on to
											// next file
					counter += 4;
					nextFile();
					continue;
				...
				addEdgeChunks();
				addInnerChunks();
			} else if (currentChunk == 124 && trimChunks.size() == 124) { // region
																			// is
																			// completely
																			// _outside_
																			// border
				...
				if (!regionFile.delete()) {
					sendMessage("Error! Region file which is outside the border could not be deleted: "
							+ regionFile.getName());
					wipeChunks();
				} else {
					// if DynMap is installed, re-render the trimmed region ...
					// disabled since it's not currently working, oh well
					// DynMapFeatures.renderRegion(world.getName(), new
					// CoordXZ(regionX, regionZ));
				}

				nextFile();
				continue;
			} else if (currentChunk == 1024) { // last chunk of the region has
												// been checked, time to wipe
												// out whichever chunks are
												// outside the border
				counter += 32;
				unloadChunks();
				wipeChunks();
			...
			// check whether chunk is inside the border or not, add it to the
			// "trim" list if not
	...
	// Advance to the next region file. Returns true if successful, false if the
	// next file isn't accessible for any reason
	...
	// add just the 4 corner chunks of the region; can determine if entire
	// region is _inside_ the border
	private void addCornerChunks() {
		regionChunks.add(new CoordXZ(CoordXZ.regionToChunk(regionX), CoordXZ
				.regionToChunk(regionZ)));
		regionChunks.add(new CoordXZ(CoordXZ.regionToChunk(regionX) + 31,
				CoordXZ.regionToChunk(regionZ)));
		regionChunks.add(new CoordXZ(CoordXZ.regionToChunk(regionX), CoordXZ
				.regionToChunk(regionZ) + 31));
		regionChunks.add(new CoordXZ(CoordXZ.regionToChunk(regionX) + 31,
				CoordXZ.regionToChunk(regionZ) + 31));
	}

	// add all chunks along the 4 edges of the region (minus the corners); can
	// determine if entire region is _outside_ the border
	private void addEdgeChunks() {
		int chunkX = 0, chunkZ;

		for (chunkZ = 1; chunkZ < 31; chunkZ++) {
			regionChunks.add(new CoordXZ(CoordXZ.regionToChunk(regionX)
					+ chunkX, CoordXZ.regionToChunk(regionZ) + chunkZ));
		}
		chunkX = 31;
		for (chunkZ = 1; chunkZ < 31; chunkZ++) {
			regionChunks.add(new CoordXZ(CoordXZ.regionToChunk(regionX)
	...
	private void wipeChunks() {
		...
		if (!regionFile.canWrite()) {
			...
			if (!regionFile.canWrite()) {
				sendMessage("Error! region file is locked and can't be trimmed: "
						+ regionFile.getName());
				return;
			}
		}

		// since our stored chunk positions are based on world, we need to
		// offset those to positions in the region file
		...
		try {
			...
			for (CoordXZ wipe : trimChunks) { // wipe this extraneous chunk's
												// pointer... note that this
												// method isn't perfect since
												// the actual chunk data is left
												// orphaned,
												// but Minecraft will overwrite
												// the orphaned data sector
												// if/when another chunk is
												// created in the region, so
												// it's not so bad
			...
			// if DynMap is installed, re-render the trimmed chunks ... disabled
			// since it's not currently working, oh well
			// DynMapFeatures.renderChunks(world.getName(), trimChunks);

			reportTrimmedChunks += trimChunks.size();
		} catch (FileNotFoundException ex) {
			sendMessage("Error! Could not open region file to wipe individual chunks: "
					+ regionFile.getName());
		} catch (IOException ex) {
			sendMessage("Error! Could not modify region file to wipe individual chunks: "
					+ regionFile.getName());
		}
		counter += trimChunks.size();
	}
	...
	private boolean isChunkInsideBorder(CoordXZ chunk) {
		return border.insideBorder(CoordXZ.chunkToBlock(chunk.x) + 8,
				CoordXZ.chunkToBlock(chunk.z) + 8);
	}
	...
	private void reportProgress() {
		...
		sendMessage(reportTrimmedRegions + " entire region(s) and "
				+ reportTrimmedChunks + " individual chunk(s) trimmed so far ("
				+ Config.coord.format(perc) + "% done" + ")");
	}
